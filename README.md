...
Added pytorch-cpu and related packages from requirements.txt to allow this to be run on non-gpu enabled systems. This enables smooth operation in any docker system, but streamlit is large enough to be detrimental to smaller systems, so your mileage may vary.

I use a custom docker image for redis-stack and a custom .env file that points directly to that docker image. This makes it possible to build with only the Docker file used. If you care to add more to the package, the requirements.txt is in the app directory.
...
All credits go to original authors. PaperIQ aims to make this slightly more accessible for more people without complex GPU setups.
...
## 💡 Learning Outcomes with PaperIQ
...
1. First, clone this repo and cd into it.
    ```bash
    $ git clone https://github.com/karthick788/PaperIQ.git && cd PaperIQ
    ```
